...
def haralick_of_image(image, underground = 0):
  
    h = haralick.Haralick(image,  underground)
    features = h.props()
    haralick_features_mean = features.mean(axis=0)
    return haralick_features_mean


def extract_haralick_features(image, path = '', round_object = False, underground = 0, rotate = False):
    if(path != ''): 
    ...
    f = []
    for angle in angles: 
        m_img = rotate_bound(image, angle)
        
        f.append(haralick_of_image(m_img, round_object=round_object, underground = underground))    
...
def extract_haralick_features(image, path = '', round_object = False, underground = 0):
    ...
    return haralick_of_image(image, underground = underground)  
...
if __name__ == "__main__":
    ...
    image1 = cv2.imread("textest/6.png")
    image2 = cv2.imread("textest/7.png")



    gray1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)
    gray2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
    gray3  = rotate_bound(gray1, 180)
    #fa = []
    
    print(extract_LBP(gray1))
    
    f1 = extract_haralick_features(gray1)
    f2 = extract_haralick_features(gray2)
    f3 = extract_haralick_features(gray3)
    f = [f1,f2]
    m  = np.mean(f, axis = 0)
    std= np.std(f, axis = 0)
    vo1 = std/abs(m) * 100
    ...
    print(np.round(f1, 3))
    print(np.round(f3,3))
    print(np.round(vo2, 3))
    
    #mean_HF_of_all_samples = pd.DataFrame(data=[f1,f3,f2,vo2, vo1], columns = columns)  
    #mean_HF_of_all_samples.to_csv('sampleText.csv')